...
class go_to_dock_server():
    # create messages that are used to publish feedback/result
    _feedback = ar_dock_motion_server.msg.DockMotionFeedback()
    _result = ar_dock_motion_server.msg.DockMotionResult()
    def __init__(self):
        ...
        self._as = actionlib.SimpleActionServer("ar_dock_motion_server", ar_dock_motion_server.msg.DockMotionAction, execute_cb=self.execute_cb, auto_start = False)
        self._as.start()
    ...
    def execute_cb(self, goal):
        self.dock_start = goal.start
        self.robot_docked = False
        if (self.dock_start == True):
            loop_rate = rospy.Rate(10) # 10hz
            while not rospy.is_shutdown() and self.robot_docked != True:
                if self.is_odom_received is True:
                    self.fnParking()
                loop_rate.sleep()

        if (self.robot_docked):
          rospy.loginfo("Docking_server : Completed task ")
          self._as.set_succeeded()
          #rospy.spin()
          #sc.playWave(path_to_sounds+"ship_bell.wav")
        else:
          rospy.loginfo("Docking_server : Fail")
    ...
    def fnSeqMovingNearbyParkingLot(self):
        if self.current_nearby_sequence == self.NearbySequence.initial_turn.value:
            if self.is_triggered == False:
                ...
                self.prev_marker_2d_pose_x = 0.0
    ...
    def fnSeqParking(self):
        ...
        if abs(self.marker_2d_pose_x) < 0.40:
            self.fnStop()
            return True
        else:
            return False
...
if __name__ == '__main__':
    rospy.init_node('go_to_dock_server')
    node = go_to_dock_server()
    node.main()
